...
def test_solve_pillar_boundaries(tmp_path: Path, caplog, monkeypatch):
    ...
    roll_pass: RollPass = sequence[-1]

    assert np.isclose(abs(roll_pass.out_profile.pillar_boundaries[-1]), sequence.out_profile.width / 2)
